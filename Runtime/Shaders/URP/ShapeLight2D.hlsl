...
#include "Packages/com.unity.render-pipelines.universal/Shaders/2D/Include/LightingUtility.hlsl" //_ShapeLightTexture

struct ShapeLightResult {
    half4 mod;
    half4 add;
};
...
ShapeLightResult CombinedShapeLight(in half4 mask, in half2 lightingUV)
{
    ShapeLightResult ret = (ShapeLightResult)0;
...
#else
    ret.mod = shapeLight0Modulate + shapeLight1Modulate + shapeLight2Modulate + shapeLight3Modulate;
    ret.add = shapeLight0Additive + shapeLight1Additive + shapeLight2Additive + shapeLight3Additive;
#endif
